...
1. **Search by topic** — run `lexi concepts <topic>` to find concepts matching
...
2. **Filter by tag** — if you know the category, run
   `lexi concepts --tag <tag>` to narrow results. Common tags are `pattern`,
   `architecture`, and `decision`.

3. **List all concepts** — run `lexi concepts --all` to get a full inventory
...
- **Wikilinks** — a wikilink like `[[Some Concept]]` can be resolved by
  running `lexi concepts "Some Concept"`. The search is fuzzy, so partial
...
- **Conventions are not concepts** — coding standards and project conventions
  ...
  `lexi conventions` to find or document conventions.
