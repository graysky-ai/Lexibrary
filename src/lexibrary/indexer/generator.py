...
_LEADING_VERB_RE = re.compile(
    r"^(?:Provides?|Acts? as|Defines?|Generates?|Coordinates?)"
    r"(?:\s+(?:a|an|the))?\s+",
    re.IGNORECASE,
)

_CLAUSE_MARKERS = (
    " that ", " so ", " which ", " without ", " keeping ",
    " used by ", " used to ",
)


def _extract_role_fragment(description: str) -> str:
    """Strip leading verb phrases and truncate at clause markers (8-word cap)."""
...
_STOP_WORDS: set[str] = {
    "the", "a", "an", "for", "in", "of", "to", "as", "by",
    "and", "or", "is", "are", "was", "were", "be", "been",
    "with", "from", "on", "at", "it", "its", "this", "that",
    "small", "stable", "simple", "single", "centralized",
    "lightweight", "reusable", "focused", "strict", "tolerant",
    "function", "utility", "helper", "module", "package", "file",
}
...
def _synthesize_summary(descriptions: list[str]) -> str:
    ...
    - Result capped at 80 chars on a word boundary.
    """
    if not descriptions:
        return ""

    fragments = [_extract_role_fragment(d) for d in descriptions]
    ...
    if len(fragments) <= 3:
        ...
        for _, idx, frag in scored:
            ...
            if kws and any(
                len(kws & sel_kw) / len(kws) > 0.5
                for sel_kw in selected_kws
                if sel_kw
            ):
                continue
...
def _get_dir_description(subdir: Path, project_root: Path) -> str:
    ...
    if child_aindex is not None:
        ...
        if child_aindex.billboard and not _is_structural_description(
            child_aindex.billboard
        ):
...
def _generate_billboard(entries: list[AIndexEntry]) -> str:
    ...
    for entry in entries:
        if entry.entry_type == "file":
            ...
            if ext:
                extensions[ext] += 1
            if not _is_structural_description(entry.description):
