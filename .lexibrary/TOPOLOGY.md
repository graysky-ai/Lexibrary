...
**Lexibrary** is an AI-friendly codebase indexer that produces `.aindex` files, per-file design documents, and a queryable symbol graph so LLM-based coding agents can navigate, understand, and edit a repository with structured context.
...
|-----------------|------|-----------|
| `lexi` | Agent-facing CLI -- lookups, search, orientation, validation, symbols, Stack/convention/design helpers | [src/lexibrary/cli/lexi_app.py](src/lexibrary/cli/lexi_app.py) |
...
Registered via `[project.scripts]` in [pyproject.toml](pyproject.toml). Both Typer apps are re-exported from [src/lexibrary/cli/__init__.py](src/lexibrary/cli/__init__.py), which also silences BAML prompt/response logging by defaulting `BAML_LOG=WARN`.
...
Lexibrarian/
  ...
  src/
    lexibrary/               -- Cross-artifact search (search.py); structured error aggregation (errors.py); exception hierarchy (exceptions.py); package metadata (__main__.py)
      archivist/             -- Stale-file detection and per-file design generation via LLM; non-LLM skeleton generator; import-target resolver; public facade
      ...
      baml_client/           -- Generated BAML client code (do not edit)
      ...
      curator/               -- Autonomous maintenance agent: coordinator, collect filters, risk taxonomy, budget tracking, hook runners, reconciliation, staleness/consistency/deprecation/migration/cascade/auditing actions, comments handling, write contract, fingerprinting
      ...
      services/              -- Domain logic extracted from CLI: lookup, design, impact, status, describe, sweep, view, symbols -- each paired with a *_render.py module for terminal formatting
  ...
  tests/                     -- Mirror of src/ layout, plus tests/fixtures/ (sample_project, curator_library)
```
...
The curator ([src/lexibrary/curator/](src/lexibrary/curator/)) is an autonomous maintenance agent invoked via `lexictl curate`. It uses a collect/triage/dispatch architecture coordinated by [coordinator.py](src/lexibrary/curator/coordinator.py): collect filters gather signals (staleness, consistency, deprecation, migration, cascade), the risk taxonomy ([risk_taxonomy.py](src/lexibrary/curator/risk_taxonomy.py)) classifies them, and actions dispatch under LLM budgets ([budget.py](src/lexibrary/curator/budget.py)) gated by an autonomy level ([config.py](src/lexibrary/curator/config.py)). [reconciliation.py](src/lexibrary/curator/reconciliation.py) merges agent-edited design files with current source; [fingerprint.py](src/lexibrary/curator/fingerprint.py) SHA-256-dedups Stack posts against the link graph; [write_contract.py](src/lexibrary/curator/write_contract.py) enforces atomic write semantics across all curator mutations.

### Symbol Graph as Second Index

The project maintains two distinct SQLite indexes with different purposes. The **link graph** ([src/lexibrary/linkgraph/](src/lexibrary/linkgraph/)) tracks wikilink references between artifacts (concepts, designs, Stack posts). The **symbol graph** ([src/lexibrary/symbolgraph/](src/lexibrary/symbolgraph/)) is a separate SQLite database tracking code-level symbols -- classes, functions, methods, and call sites resolved across files. Do not conflate them: [symbolgraph/query.py](src/lexibrary/symbolgraph/query.py) is read-only and schema-version-checked, [symbolgraph/builder.py](src/lexibrary/symbolgraph/builder.py) rebuilds from AST parser output, and cross-file resolution happens in [resolver_python.py](src/lexibrary/symbolgraph/resolver_python.py) using [python_imports.py](src/lexibrary/symbolgraph/python_imports.py). `lexi symbols` (service: [services/symbols.py](src/lexibrary/services/symbols.py)) is the agent-facing query surface.
...
| [src/lexibrary/services/symbols.py](src/lexibrary/services/symbols.py) | Symbol graph query service backing `lexi symbols` |
...
| [src/lexibrary/config/schema.py](src/lexibrary/config/schema.py) | Pydantic v2 config schema with defaults for daemon, crawler, LLM, token budgets, topology, symbolgraph |
