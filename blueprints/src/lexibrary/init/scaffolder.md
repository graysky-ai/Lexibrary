...
| `_ensure_daemon_files_gitignored` | `(project_root: Path) -> bool` | Append `.lexibrary.log`, `.lexibrary.pid`, and `.lexibrary/index.db` to `.gitignore` if not already present; creates `.gitignore` if it does not exist |
...
- Both init paths call `_ensure_daemon_files_gitignored()` to add `.lexibrary.log` and `.lexibrary.pid` to `.gitignore`
- `_DAEMON_GITIGNORE_PATTERNS` constant holds the list of daemon runtime files and generated artifacts (`.lexibrary.log`, `.lexibrary.pid`, `.lexibrary/index.db`) to gitignore
