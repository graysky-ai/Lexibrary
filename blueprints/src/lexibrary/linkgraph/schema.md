...
| --- | --- | --- |
| `SCHEMA_VERSION` | `int` (currently `3`) | Schema version stored in `meta` table; mismatch on open triggers a full rebuild |
...
| 6 | `conventions` | Conventions scoped to directories: `directory_path`, `ordinal`, `body`, `source` (user/agent/config), `status` (draft/active/deprecated), `priority` (int); UNIQUE on (directory_path, ordinal) |
...
The schema creates 13 secondary indexes for query performance:
...
- `aliases`: artifact_id
- `conventions`: directory_path, status
