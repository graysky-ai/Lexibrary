...
**Summary:** Loads and shallow-merges global (`~/.config/lexibrary/config.yaml`) and project (`.lexibrary/config.yaml`) YAML files into a validated `LexibraryConfig`.
...
| `GLOBAL_CONFIG_PATH` | `Path` | Default global config path (`~/.config/lexibrary/config.yaml`); respects `XDG_CONFIG_HOME` |
...
- `lexibrary.config.__init__` — re-exports `load_config`
- `lexibrary.daemon.service` — calls `load_config`
