...
**Summary:** Git post-commit hook installation for Lexibrary -- installs a hook that runs `lexictl update --changed-only` in the background after each commit, so design files stay in sync without blocking the developer.

## Interface

| Name | Signature | Purpose |
| --- | --- | --- |
| `install_post_commit_hook` | `(project_root: Path) -> HookInstallResult` | Install or update the Lexibrary post-commit hook; idempotent |
| `HookInstallResult` | `@dataclass` | Result with flags: `installed`, `already_installed`, `no_git_dir`, `message` |
| `HOOK_MARKER` | `str` | Marker comment (`# lexibrary:post-commit`) used for idempotent detection |
...
- `lexibrary.cli.lexictl_app` -- `setup --hooks` command calls `install_post_commit_hook`

## Key Concepts

- Hook script uses `git diff-tree --no-commit-id --name-only -r HEAD` to list changed files
- Output is redirected to `.lexibrary.log` and run in the background (`&`) so it never blocks `git commit`
- Three behaviours:
