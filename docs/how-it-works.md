...
Alongside the link graph, Lexibrary builds a second SQLite database (`.lexibrary/symbols.db`) that records relationships **inside** files: function and method definitions, the call edges between them, class hierarchy edges, and the external/dynamic calls that can't be resolved to a project symbol. Where the link graph answers "which files import this file?", the symbol graph answers "which functions call this function?" and "which classes inherit from this class?".
...
**Resolution.** Once every definition is written to the database, a second pass resolves call sites. The Python resolver (`resolver_python.py`) handles free functions in the same file, `self.method()` inside a class, `from module import name`, `import module`, and relative imports, sharing its module-to-file logic with `archivist/dependency_extractor.py` (the `python_imports` table) so the symbol graph and the link graph agree on where an imported name lives. TypeScript and JavaScript resolution is intra-file fuzzy matching only until Phase 6 adds `tsconfig.json`-aware resolvers. Calls that can't be mapped to a definition — standard library calls, dynamic dispatch — are stored in `unresolved_calls` rather than dropped.

**Class edges.** Phase 3 adds a third pass that resolves class hierarchy. Parsers emit `ClassEdgeSite` entries for `inherits` edges (Python `class Foo(Bar):`, TS/JS `extends`, and TS `implements`) and `instantiates` edges (Python PascalCase calls matching `^[A-Z][A-Za-z0-9]*$` and TS/JS `new` expressions). The builder's pass 3 resolves each target name through the same import resolver used by the call pass, filtered to `symbol_type='class'` for `instantiates` edges so calls to PascalCase functions don't pollute the graph. Resolved edges land in `class_edges`; external bases like Pydantic `BaseModel` or stdlib `Enum` go into `class_edges_unresolved`. A side-effect of having these edges is that the Python resolver can now walk the MRO for `self.method()` calls — if a subclass method calls `self.foo()` and `foo` is defined only on a base class, the call resolves to the base class definition via a BFS through `class_edges` (`_resolve_self_method_with_mro`), with a diamond-inheritance visited set to prevent cycles.

**Consumption.** Three commands read the symbol graph. `lexi trace <symbol>` shows callers, callees, unresolved calls, and — for classes — base classes, subclasses and instantiation sites, and unresolved external bases. `lexi search --type symbol <query>` fuzzy-finds a symbol by name or qualified name; and `lexi lookup <file> --full` appends a "Key symbols" section listing the file's public functions, classes, and methods with inbound and outbound call counts plus a Phase 3 "Class hierarchy" table listing every class with its resolved bases, unresolved external bases, subclass count, and method count. Individual files are refreshed on `lexi design update <file>`; a full rebuild happens during `lexictl update`.
