...
| `AGENT_UPDATED` | The design file body was modified since last generation (e.g. by a maintainer) | Footer hashes are refreshed, but the LLM is **not** called (preserves modifications) |
...
Lexibrary stores a `design_hash` in the metadata footer of each design file. This hash covers the frontmatter and body (excluding the footer itself). When the design file body is modified after generation -- whether by a maintainer, an automated tool, or any other process -- the computed hash of the current content will differ from the stored `design_hash`, triggering the `AGENT_UPDATED` classification. This design-hash mismatch detection ensures that manual edits are never silently overwritten by LLM regeneration.
...
| `files_agent_updated` | Files where body modifications were preserved (not overwritten) |
