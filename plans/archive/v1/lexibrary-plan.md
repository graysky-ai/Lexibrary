# Lexibrary / Lexibrary — Implementation Plan
...
- CLI (`lexibrary init|crawl|daemon|status|clean`) via Typer — aliased as both `lexibrary` and `lexi`
...
│   └── lexibrary/
│       ├── __init__.py
│       ├── __main__.py          # python -m lexibrary
...
Both `lexibrary` and `lexi` work as CLI entry points (registered in `pyproject.toml` `[project.scripts]`).
...
**Milestone:** `uv run python -m lexibrary --help` works.
...
**Files:** crawler/*
**Milestone:** `lexibrary crawl` produces `.aindex` files for a project.
...
**Files:** daemon/*
**Milestone:** `lexibrary daemon` watches and incrementally updates.
