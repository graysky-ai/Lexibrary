...
> **Date:** 2026-02-24 (revised 2026-02-24 post agent-navigation & cli-command-rebalance)
...
| AU4 | Agent handoff signals (IWH) | **Infrastructure only** | Model/parser/serializer done; no code reads/writes them automatically |
...
- `.iwh` check in orient skill only checks `.lexibrary/.iwh`, but rules say "any directory" — inconsistency
...
Agent A leaves work incomplete:
  (Rules say) Create .iwh file in relevant directory

Agent B starts session:
  (Rules say) Check for .iwh, read, act on instructions, delete

OR via /lexi-orient:
  Step 2: Check for .lexibrary/.iwh
```

**Gaps** (consolidated as **C3** in Section 7)**:**
- **IWH is fully built but completely unused in production** — no code writes or reads .iwh files
- No `lexi iwh write <message>` or `lexi iwh read` commands
- Agent must create .iwh files manually (know format, create file)
- Orient skill only checks `.lexibrary/.iwh`, not per-directory signals
- No `lexi iwh list` to find all .iwh files across project
- Gitignore integration exists but no CLI surface
- Archivist doesn't check for .iwh "don't regenerate" signals
...
| Leave handoff signals | Manual .iwh file creation | **Needs CLI** |
...
| IWH write | `lexi iwh write <msg>` (agent handoff) | Not needed | **Missing** |
| IWH read/list | `lexi iwh read` / `lexi iwh list` | `lexictl iwh clean` (cleanup) | **Missing** |
...
| **IWH integration** | Archivist should check for .iwh signals and include them in design file generation context | Low |
...
3. **The archivist should be aware of IWH signals** — if an .iwh file says "refactoring in progress, don't regenerate", the archivist should respect that.
...
| C4 | ~~Rules reference non-existent commands~~ | ~~Agent trust undermined~~ | `base.py` updated: `lexi validate` in After Editing, `lexi status` in orient skill now valid | **RESOLVED** (except IWH, see C3) |
...
### lexictl (Operator-Facing) — 8 commands
...
3. **IWH awareness** — archivist should check for .iwh "don't regenerate" signals
