...
6. After all files: call `generate_topology()` to produce `.lexibrary/TOPOLOGY.md`
...
#### Scenario: Topology generated after design files
- **WHEN** `update_project()` completes file processing
- **THEN** `generate_topology()` SHALL be called to produce `.lexibrary/TOPOLOGY.md`

#### Scenario: Link graph built after design files and topology
- **WHEN** `update_project()` completes
- **THEN** the link graph index SHALL be built after all design files are generated and TOPOLOGY.md is generated
...
- **AND** `TOPOLOGY.md` SHALL NOT be regenerated
