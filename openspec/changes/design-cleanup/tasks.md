...
- [ ] 20.1 Run `lexi design update --force src/lexibrary/` with sandbox override. This is the second full re-render (first was Group 6 post-§2.3). Expected effect: Part 1 serializer noise gone on every file; aggregator files compacted; Complexity Warning filter dropped generic-hedge cases.
- [ ] 20.2 Diff total design-file body size (excluding footer) before vs. after. Reference: `find .lexibrary/designs/src/lexibrary -name '*.md' | xargs wc -l | tail -1`. Record the before/after line counts in the commit message. Expected drop: 20–40% for non-aggregator files, dramatically more for aggregators.
- [ ] 20.3 Re-read the 6 files seeded by the plan (`errors.py.md`, `linkgraph/builder.py.md`, `cli/_format.py.md`, `services/sweep.py.md`, `symbolgraph/resolver_js.py.md`, `artifacts/__init__.py.md`). Confirm each shows visible signal-to-noise improvement — aggregator compacted, doubled fences gone, meta footer compact, status omitted from frontmatter, Dependents list populated where the module was actually referenced.
- [ ] 20.4 Run `uv run pytest tests/ -v` with sandbox override. All tests green.
- [ ] 20.5 Run `uv run mypy src/` with sandbox override. Clean.
- [ ] 20.6 Run `uv run ruff check src/ tests/` with sandbox override. Clean.
- [ ] 20.7 Run `lexi validate` with sandbox override. Confirm no new error-severity issues vs. pre-change baseline. Dangling-wikilink hits ARE expected where the wikilink prompt tightening (Group 11) took effect — these are genuine signal, not regressions.
- [ ] 20.8 Post-change spot-checks for `Dependents` accuracy (from the plan's post-§2.3 acceptance): the 13 previously-`(none)` service design files should all show realistic reverse edges after Group 6. Confirm a second time here — the second force re-render should not have regressed any of them.
- [ ] 20.9 Commit the final re-render. Title: "design-cleanup: final force re-render (full Part 1 + Part 2)". Body: summarise the delta (before/after line counts, Complexity Warning drop ratio, aggregator count, Insights cleanup totals).
