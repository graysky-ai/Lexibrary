...
- [x] 3.1 Implement `check_dangling_links()` in `src/lexibrary/validator/checks.py`. Opens `index.db`, queries all artifacts with `kind` in (`source`, `design`, `concept`, `stack`), verifies backing file exists at `project_root / artifact.path`. Skips `convention` artifacts. Reports missing files at info severity.
...
- [x] 4.1 Implement `check_orphan_artifacts()` in `src/lexibrary/validator/checks.py`. Opens `index.db`, queries all non-convention artifacts, verifies backing file exists. Reports artifacts for deleted files at info severity with suggestion to rebuild index.
...
- [x] 5.1 Add `check_bidirectional_deps`, `check_dangling_links`, and `check_orphan_artifacts` to imports in `src/lexibrary/validator/__init__.py`.
...
- [x] 6.1 Add link graph health section to the full dashboard in `lexictl status` in `src/lexibrary/cli/lexictl_app.py`. Use `read_index_health()` to get counts and timestamp. Display `Link graph: N artifacts, M links (built <timestamp>)` or `Link graph: not built (run lexictl update to create)`. Place after the Stack section and before the Issues section.
...
- [x] 7.1 Update the blueprint/design file for `src/lexibrary/validator/checks.py` to document the three new check functions.
- [x] 7.2 Update the blueprint/design file for `src/lexibrary/cli/lexictl_app.py` to document the link graph health section in status.
- [x] 7.3 Update the blueprint/design file for `src/lexibrary/linkgraph/` to document the `read_index_health()` helper.

## 8. Final Verification

- [x] 8.1 Run full test suite (`uv run pytest --cov=lexibrary`) and confirm all tests pass with no regressions.
