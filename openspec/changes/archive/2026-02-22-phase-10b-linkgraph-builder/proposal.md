...
- Implement `IndexBuilder` class in `src/lexibrarian/linkgraph/builder.py` with two primary methods:
...
- Update `src/lexibrarian/linkgraph/__init__.py` to expose `IndexBuilder` and the `build_index()` / `open_index()` public API functions
...
- **New file:** `src/lexibrarian/linkgraph/builder.py` (new module)
- **Modified file:** `src/lexibrarian/linkgraph/__init__.py` (new exports)
