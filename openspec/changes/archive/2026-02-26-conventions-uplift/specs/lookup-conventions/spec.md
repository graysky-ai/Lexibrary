...
**Reason**: Replaced by scope-based grouping. Conventions are now standalone files with explicit `scope` fields rather than being embedded in `.aindex` files per directory.
**Migration**: Convention display now groups by the `scope` frontmatter field. The visual format is similar but the data source is `ConventionIndex` instead of `.aindex` parent-directory walk.

### Requirement: Missing aindex files are silently skipped
**Reason**: Convention retrieval no longer walks `.aindex` files. Conventions are loaded from `.lexibrary/conventions/` via `ConventionIndex`.
**Migration**: `ConventionIndex.load()` handles missing directories gracefully by returning an empty list.
