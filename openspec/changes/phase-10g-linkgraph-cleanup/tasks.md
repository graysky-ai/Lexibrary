...
- [x] 1.1 Update `serialize_design_file()` in `src/lexibrary/artifacts/design_file_serializer.py` to emit the annotation line `*(see \`lexi lookup\` for live reverse references)*` after the `## Dependents` heading, before the bullet items or `(none)`
...
- [x] 2.2 Update `_ensure_daemon_files_gitignored()` in `src/lexibrary/init/scaffolder.py` (or create a dedicated helper) to include `.lexibrary/index.db` in the patterns ensured during `lexictl init`
...
- [x] 3.1 Create `blueprints/src/lexibrary/linkgraph/__init__.md` describing the module's public API re-exports
- [x] 3.2 Create `blueprints/src/lexibrary/linkgraph/schema.md` describing DDL constants, `ensure_schema()`, `check_schema_version()`, `set_pragmas()`, `SCHEMA_VERSION`, and the 8-table + FTS5 structure
- [x] 3.3 Create `blueprints/src/lexibrary/linkgraph/builder.md` describing the `IndexBuilder` class, `full_build()`, `incremental_update()`, and build pipeline
- [x] 3.4 Create `blueprints/src/lexibrary/linkgraph/query.md` describing the `LinkGraph` read-only query interface, key methods, `traverse()`, and graceful degradation
...
- [x] 6.1 Search for TODO comments in `src/lexibrary/linkgraph/` and resolve or remove them
- [x] 6.2 Search for Phase 10-related TODOs in integration points (`archivist/pipeline.py`, `cli/`, `validator/`)
- [x] 6.3 Run the full test suite: `uv run pytest --cov=lexibrary` and verify `linkgraph/` module coverage
